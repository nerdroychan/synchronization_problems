...
// Mutex for strictly exclusive writing
...
void* reader(int* id) {
    while (1) {
        // Make sure no writer is working
        ...
        // Now the permission is gained
        pthread_mutex_unlock(&lock);
        // Release the write lock
        pthread_mutex_unlock(&wlock);

        // Start to read, now this thread is either:
        // - The first reader and has gain the write lock
        ...
        pthread_mutex_lock(&lock);
        readers--;
        // If no more readers, release the write lock
        if (readers == 0) {
